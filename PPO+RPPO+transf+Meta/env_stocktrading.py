...
class StockTradingEnv(gym.Env):
    ...
    def step(self, actions):
        ...
        if self.terminal:
            ...
            # Convert actions to normalized portfolio weights
            actions = np.clip(actions, 0, 1)
            actions = actions / (np.sum(actions) + 1e-8)  # normalize to sum=1
            ...
            # Sharpe-ratio style reward
            self.rewards_memory.append(end_total_asset - begin_total_asset)
            if len(self.rewards_memory) >= 20:  # rolling 20-step window
                returns = np.array(self.rewards_memory[-20:])
                mean_ret = returns.mean()
                std_ret = returns.std() + 1e-6
                self.reward = (mean_ret / std_ret) * self.reward_scaling
            else:
