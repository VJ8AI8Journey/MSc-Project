...
MODELS = {"a2c": A2C, "ddpg": DDPG, "td3": TD3, "sac": SAC, "ppo": PPO, "recurrent_ppo": RecurrentPPO}

MODEL_KWARGS = {}
for x in MODELS.keys():
    MODEL_KWARGS[x] = config.__dict__.get(f"{x.upper()}_PARAMS", {})
...
class DRLAgent:
    ...
    def get_model(
        ...
        # In DRLAgent.get_model(...)
        if model_name == "recurrent_ppo":
...
class DRLEnsembleAgent:
    ...
    def run_ensemble_strategy(
        ...
        # Model Sharpe Ratios
        model_list = ["a2c", "ppo", "ddpg", "sac", "td3"]
        model_dct = {k: {"sharpe_list": [], "sharpe": -1} for k in model_list}
        ...
        for i in range(
            ...
            sharpes = [model_dct[k]["sharpe"] for k in model_list]
            max_mod = model_list[np.argmax(sharpes)]
